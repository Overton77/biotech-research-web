...
import { useMission, useMissionStatus } from "@/lib/queries";
...
export function MissionLivePanel({ missionId }: { missionId: string }) {
  ...
  useEffect(() => {
    ...
    const onProgress = (data: ResearchProgressPayload) => {
      ...
      if (p.subagent_name) {
        setCurrentSubagent(
          data.event_type === "agent_completed" ? undefined : p.subagent_name,
        );
      }
    };
